...
class CreateClientWindow(Toplevel, CenterWidgetMixin):
    ...
    def build(self):
        ...
        Label(frame, text="DNI (8 numeros y 1 letra)").grid(row=0, column=0)
        Label(frame, text="Nombre (3 a 30 chars)").grid(row=0, column=1)
        Label(frame, text="Apellido (3 a 30 chars)").grid(row=0, column=2)
...
class MainWindow(Tk, CenterWidgetMixin):
    ...
    def build(self):
       ...
       scrollball = Scrollbar(frame)
       scrollball.pack(side=RIGHT, fill=Y)

       treeview['yscrollcommand'] = scrollball.set
